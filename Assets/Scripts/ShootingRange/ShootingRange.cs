...
public class ShootingRange : MonoBehaviour
{
    ...
    public float Accuracy { get => Shots > 0 ? (Hits / (float)Shots) : 0f;  }

    public double RuntimeTotalDuration { get; private set; }
    public double RuntimeMinDuration { get; private set; }
    public double RuntimeMaxDuration { get; private set; }
    public int NumCbkCalls { get; private set; }

    public float RuntimeAvgDuration {
        get => NumCbkCalls > 0 ? (float)(RuntimeTotalDuration / (double)NumCbkCalls) : 0f; }
    ...
    [SerializeField]
    bool enforceSpeedLimit = false;

    public void INTERNAL_setEnforceSpeedLimit(bool val)
    {
        enforceSpeedLimit = val;
    }
    ...
    // [SerializeField]
    // bool DB_overSpeedLimit = false;

    [SerializeField]
    double DEBUG_RuntimeTotalDuration;
    [SerializeField]
    double DEBUG_RuntimeMinDuration;
    [SerializeField]
    double DEBUG_RuntimeMaxDuration;
    [SerializeField]
    double DEBUG_NumCbkCalls;
    [SerializeField]
    double DEBUG_RuntimeAvgDuration;
    ...
    public void INTERNAL_ResetStats()
    {
        ...
        RuntimeTotalDuration = 0d;
        RuntimeMinDuration = double.MaxValue;
        RuntimeMaxDuration = 0d;
        NumCbkCalls = 0;
}
...
void PrintAvg()
    ...
    void FixedUpdate()
    {
        ...
        if (ProjectilePool.Count > 0 && Time.timeSinceLevelLoad > (LastShotTime + CoolOffTime))
        {
            ...
            double cbkStartTime = Time.realtimeSinceStartupAsDouble;

            // Calc the shot trajectory, if possible
            cbkRes = cbk(LaunchPos, ShotSpeed, Physics.gravity,
                Target.transform.position, Target.rbody.linearVelocity, Target.transform.forward,
                maxAllowedErrorDist,
                out var projectileDir, out var projectileSpeed, out var t, out var altT);

            double cbkEndTime = Time.realtimeSinceStartupAsDouble;

            double cbkRuntime = cbkEndTime - cbkStartTime;

            RuntimeTotalDuration += cbkRuntime;
            ++NumCbkCalls;

            if (cbkRuntime < RuntimeMinDuration)
                RuntimeMinDuration = cbkRuntime;
            if (cbkRuntime > RuntimeMaxDuration)
                RuntimeMaxDuration = cbkRuntime;


            DEBUG_RuntimeTotalDuration = RuntimeTotalDuration;
            DEBUG_RuntimeMinDuration = RuntimeMinDuration;
            DEBUG_RuntimeMaxDuration = RuntimeMaxDuration;
            DEBUG_NumCbkCalls = NumCbkCalls;
            DEBUG_RuntimeAvgDuration = RuntimeAvgDuration;
            ...
            if (cbkRes)
            {
                ...
                // can get there
                var intercept = Target.transform.position + Target.rbody.linearVelocity * t;
                ...
                if (Mathf.Abs(intercept.x) <= Target.AbsXRange)
                {
                    ...
                    var prjDirMag = projectileDir.magnitude;
                    DB_launchDirMagnitude = prjDirMag;

                    float clampedSpeed = projectileSpeed;

                    if(projectileSpeed > ShotSpeed)
                    {
                        // DB_overSpeedLimit = true;
                        clampedSpeed = ShotSpeed;
                    }
                    // else
                    // {
                    //     DB_overSpeedLimit = false;
                    // }
                    ...
                    p.rbody.linearVelocity = Vector3.zero;
                    p.rbody.angularVelocity = Vector3.zero;

                    Vector3 throwVec;
                    if (enforceSpeedLimit)
                    {
                        Vector3 prjDir = Vector3.zero;
                        if (!Mathf.Approximately(prjDirMag, 0f))
                            prjDir = projectileDir / prjDirMag;
                        throwVec = prjDir * clampedSpeed;
                    }
                    else
                    { 
                        throwVec = projectileDir * projectileSpeed;
                    }
